#!/usr/bin/env python3
"""
Script to fix inconsistent chord data before applying new migration
"""
...
def fix_chord_data():
    ...
    # Step 3: Validate all chords
    print("\n3. Validating chord data...")
    invalid_chords = []
    
    for chord in Chord.objects.all():
        # Check if chord name matches root and quality
        quality_suffix = quality_map.get(chord.quality, '')
        expected_name = f"{chord.root}{quality_suffix}"
        
        if chord.chord_name != expected_name:
    ...
    if invalid_chords:
        print(f"  ⚠️  Found {len(invalid_chords)} still invalid chords:")
        for chord in invalid_chords[:5]:  # Show first 5
            print(f"    ID {chord['id']}: {chord['chord_name']} (expected: {chord['expected']})")
    else:
        print("  ✅ All chords are now valid")
    
    print(f"\n🎉 Data fix complete!")
    print(f"   - Fixed {fixed_names} chord names")
    print(f"   - Fixed {fixed_timing} timing conflicts")
    print(f"   - {len(invalid_chords)} chords still need attention")
